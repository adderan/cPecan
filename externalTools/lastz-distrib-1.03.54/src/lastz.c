...
#include "seed_prob_table.h"    // seed probability table
...
static const control defaultParams =
	...
	NULL,								// statsFile
	NULL,								// statsFilename
	NULL,								// seedSamplingProbabilitiesFile
	NULL,								// seedSamplingProbabilitiesFilename
...
int main
   (int				argc,
    ...
    seedprobtable*  seedProbTable = NULL;
...
next_target:
    ...
    if (currParams->seedSamplingProbabilitiesFile) {
        seedProbTable = load_prob_table(currParams->seedSamplingProbabilitiesFile, 
	...
	while (true)
		...
		abortQuery = !start_one_strand (target, targPositions, seedProbTable, query,
...
int start_one_strand
   (seq*			target,
	...
	if (query->fileType == seq_type_qdna)
		...
		seed_hit_search (target, targPositions, spt,
		...
		basesHit = seed_hit_search (target, targPositions, spt,
		                            query, 0, query->len, currParams->selfCompare,
		                            currParams->upperCharToBits, currParams->hitSeed,
		                            searchLimit,
		                            (currParams->searchLimitWarn)? currParams->searchLimit : 0,
...
static void parse_options_loop
   (int			argc,
	...
	while (argc > 0)
		...
		// --out[put]=<file>
		if (strcmp_prefix (arg, "--seedSamplingProbabilities=") == 0)
			{
			if (lzParams->seedSamplingProbabilitiesFilename != NULL) goto duplicated_option;
			lzParams->seedSamplingProbabilitiesFilename = copy_string (argStr);
			goto next_arg;
			}
...
static void parse_options
	...
	*lzParams = defaultParams;
    ...
    if (lzParams->seedSamplingProbabilitiesFilename != NULL)
    {
        lzParams->seedSamplingProbabilitiesFile = fopen_or_die (lzParams->seedSamplingProbabilitiesFilename, "r");
    }
